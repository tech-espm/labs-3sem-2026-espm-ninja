...
export class Game extends Phaser.Scene
{
    ...
    addFoodWave ()
    {
        ...
        // Diminui o intervalo de tempo entre o aparecimento das frutas baseado na pontuação
        const minDel = Math.max(150, 500 - this.score * 1.5);
        const maxDel = Math.max(400, 1000 - this.score * 2);
        const randomInterval = Phaser.Math.Between(minDel, maxDel);

        // Aumenta a gravidade progressivamente conforme a pontuação sobe (começa com 400)
        const currentGravity = 400 + (this.score * 2);
        this.physics.world.gravity.y = Math.min(currentGravity, 1200);
